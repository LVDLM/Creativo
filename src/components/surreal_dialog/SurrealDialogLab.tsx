...
export const SurrealDialogLab: React.FC<SurrealDialogLabProps> = ({ 
  ...
  return (
    <div className="relative p-4 md:p-8 max-w-full overflow-hidden">
      {/* Decorative Bubbles */}
      ...
      <img 
        src="/Globo1.png" 
        alt="" 
        className="absolute top-20 -right-16 md:right-0 w-40 md:w-64 opacity-20 md:opacity-30 -z-10 pointer-events-none animate-float transition-all"
        aria-hidden="true"
      />
      <img 
        src="/Globo2.png" 
        alt="" 
        className="absolute bottom-10 -left-16 md:left-0 w-40 md:w-64 opacity-20 md:opacity-30 -z-10 pointer-events-none animate-float-delayed transition-all"
        aria-hidden="true"
      />
      ...
      <header className="w-full max-w-5xl flex flex-col sm:flex-row justify-between items-center mb-8 md:mb-16 gap-6 z-10 relative">
        <div className="flex items-center gap-4 md:gap-6">
          <div className="p-3 md:p-5 bg-indigo-600 rounded-[2rem] md:rounded-[2.5rem] shadow-[4px_4px_0px_rgba(0,0,0,1)] md:shadow-[8px_8px_0px_rgba(0,0,0,1)] border-2 border-black transform -rotate-3 hover:rotate-0 transition-transform cursor-pointer group">
            <Sparkles className="w-6 h-6 md:w-10 md:h-10 text-white group-hover:scale-125 transition-transform" />
          </div>
          {renderSurrealTitle()}
        </div>
        ...
        <button 
          ...
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-white hover:bg-slate-50 rounded-2xl transition-all border-2 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-1 active:translate-y-1"
        >
          <span className="text-sm font-black uppercase text-slate-900 tracking-wider">Ayuda</span>
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col items-center z-10 relative">
        {/* Controls */}
        <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border-2 border-black w-full max-w-2xl mb-8 md:mb-16 shadow-[6px_6px_0px_rgba(99,102,241,0.2)] md:shadow-[10px_10px_0px_rgba(99,102,241,0.2)]">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-8 md:gap-10">
            <div className="flex-1 space-y-4">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest block text-center md:text-left">
                ¿Cuántos hablan?
              </label>
              <div className="flex gap-2 md:gap-3 justify-center md:justify-start flex-wrap">
                {[2, 3, 4, 5, 6].map(num => (
                  <button
                    ...
                    disabled={isSpinning}
                    className={`w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl font-black text-base md:text-lg border-2 border-black transition-all ${
                      numInterlocutors === num 
                        ? 'bg-indigo-600 text-white shadow-[4px_4px_0px_rgba(0,0,0,1)] -translate-y-1' 
                        : 'bg-slate-50 text-slate-400 hover:bg-slate-100 active:shadow-none'
                    }`}
                  >
                    {num}
                  </button>
            ...
            <button
              onClick={handleSpin}
              disabled={isSpinning}
              className={`flex items-center justify-center gap-3 px-8 md:px-10 py-4 md:py-5 rounded-xl md:rounded-[2rem] border-2 border-black font-black text-lg md:text-xl transition-all transform active:scale-95 active:shadow-none ${
                isSpinning 
                  ? 'bg-slate-200 text-slate-400 cursor-not-allowed' 
                  : 'bg-yellow-400 text-black shadow-[4px_4px_0px_rgba(0,0,0,1)] md:shadow-[6px_6px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:bg-yellow-300'
              }`}
            >
              {isSpinning ? (
                <>
